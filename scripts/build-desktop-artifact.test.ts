...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Glassycode");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Glassycode (Candidate)");
  });
  ...
  it.effect("prefers a generic desktop update URL for self-hosted releases", () =>
    Effect.gen(function* () {
      const config = yield* resolveGenericPublishConfig();
      ...
      Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: { T3CODE_DESKTOP_UPDATE_URL: "https://updates.example.com/desktop" },
          }),
        ),
      ),
    ),
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.heyglassy.glassycode",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.heyglassy.glassycode</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.heyglassy.glassycode");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/t3code.provisionprofile");
      assert.deepStrictEqual(mac.protocols, [
        { name: "Glassycode", schemes: ["glassycode", "glassycode-dev"] },
      ]);
    }).pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })))),
  );
