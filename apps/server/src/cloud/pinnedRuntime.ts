...
/**
 * A pinned runtime is an exact `@heyglassy/t3@<version>` npm-installed into
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "@heyglassy", "t3", "dist", "bin.mjs"),
...
/**
 * Installs `@heyglassy/t3@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "@heyglassy", "t3", "dist", "bin.mjs"),
