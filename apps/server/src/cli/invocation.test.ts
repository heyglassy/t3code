...
it("re-suggests the nightly channel only for nightly builds", () => {
  assert.equal(suggestedPackageSpec("0.0.31-nightly.20260729"), "@heyglassy/t3@candidate");
  assert.equal(suggestedPackageSpec("0.0.31"), "@heyglassy/t3");
});
...
it("formats serve suggestions to match the launching command", () => {
  assert.equal(
    ...
    "npx @heyglassy/t3@candidate serve",
  );
  assert.equal(
    formatCliCommand({
      subcommand: "serve",
